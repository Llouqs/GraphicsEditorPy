...
class FramedObjSelection(Selection):
    ...
    def try_grab_marker(self, x, y):
        ...
        """
        if self.x1 + 6 > x > self.x1 - 6 and self.y1 + 6 > y > self.y1 - 6:
            self.drag = 1
            return True

        if self.x2 + 6 > x > self.x2 - 6 and self.y2 + 6 > y > self.y2 - 6:
            self.drag = 2
            return True

        if self.x3 + 6 > x > self.x3 - 6 and self.y3 + 6 > y > self.y3 - 6:
            self.drag = 3
            return True

        if self.x4 + 6 > x > self.x4 - 6 and self.y4 + 6 > y > self.y4 - 6:
            self.drag = 4
            return True
